...
char_ix = {a:i for i,a in enumerate(chars)}
ix_char = {i:a for i,a in enumerate(chars)}
...
X = [[char_ix[text[i+j]] for j in range(seq_len)] for i in range(len(text)-seq_len-1)]
Y = [[char_ix[text[i+j]] for j in range(seq_len)] for i in range(1,len(text)-seq_len)]
...
emb_mtx = tf.Variable(tf.random_normal((len(chars), emb_size), 0, 0.01), name='emb_mtx')
...
with tf.Session() as sess:
    ...
    while True:
        ...
        if epoch % 70 == 0:
            ...
            sample = [char_ix[x] for x in list(seed)]
            ...
            verbalization = [ix_char[i] for i in sample]
            print("Sample:\n",''.join(verbalization)) 
